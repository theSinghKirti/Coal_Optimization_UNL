...
REAL_PDF_PATH = os.path.abspath(
    os.path.join(
        os.path.dirname(__file__),
        "..",
        "test_data",
        "AnparaTPS-LandedCost-(16-31)March26 (1).pdf"
    )
)
...
def test_oversized_file_rejected(client):
    # Generates a mock byte stream larger than 35 MB to verify rejection
    large_data = b"x" * (35 * 1024 * 1024 + 1)
    resp = client.post(
        "/api/v1/documents",
        data={"document_type": "LANDED_COST_DOCUMENT"},
        files={"file": ("too_large.pdf", large_data, "application/pdf")},
    )
