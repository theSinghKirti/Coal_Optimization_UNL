...
def review_constraint(db: Session, record_id: uuid.UUID, payload: object) -> FSAConstraint:
    ...
    action = "FSA_CONSTRAINT_APPROVED" if record.status == "APPROVED" else "FSA_CONSTRAINT_REJECTED"
    
    before_plant = str(before["plant_id"]) if before["plant_id"] else None
    after_plant = str(record.plant_id) if record.plant_id else None
    
    if before_plant != after_plant:
        audit_service.record(
            db,
            entity_type="fsa_constraint",
            entity_id=record.id,
            action="FSA_CONSTRAINT_MAPPING_CHANGED",
            document_id=record.document_id,
            before={"plant_id": before_plant},
            after={"plant_id": after_plant},
